...
export function RegistroPostaTab() {
  const clients = useClients();
  const next = usePostaReceiptNextNumber();
  const create = useCreatePostaReceipt();
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setOkMsg(`Recibo ${formatRP(created.receiptNumber)} registrado.`);
      setClientId('');
      ...
      const message =
        (err as { response?: { data?: { message?: string | string[] } } })
          ?.response?.data?.message ?? 'No se pudo registrar el recibo.';
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Registrar orden recibo en posta</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={submit} className="space-y-5">
          <div className="rounded-lg border border-border bg-muted/30 p-4">
            ...
            <p className="mt-1 text-3xl font-semibold tabular-nums text-primary">
              {next.data ? formatRP(next.data.next) : '—'}
            </p>
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              value={status}
              onChange={(e) => setStatus(e.target.value as DispatchOrderStatus)}
            >
